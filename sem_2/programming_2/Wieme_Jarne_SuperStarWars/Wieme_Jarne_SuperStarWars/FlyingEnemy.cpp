...
FlyingEnemy::FlyingEnemy(const Point2f& bottomLeftStartPoint, float scale, int health, TextureManager& textureManager)
...
{
	m_pSprites.push_back(new Sprite{ textureManager.GetTexture("Resources/Enemies/Enemy2Fly.png"), Sprite::AnimType::loop, 4, 1, 5.f });
	m_pSprites.push_back(new Sprite{ textureManager.GetTexture("Resources/Enemies/Enemy2Attack.png"), Sprite::AnimType::dontRepeat, 3, 1, 0.f });
...
void FlyingEnemy::Update(float elapsedSec, const Level& level, const Avatar& avatar)
{
	if (m_Health <= 0)
	{
...
void FlyingEnemy::CheckActionState(const Avatar& avatar)
{
	...
	if (m_Shape.bottom < m_StartPos.y || m_SecondsAfterAttack >= 1.f)
	{
